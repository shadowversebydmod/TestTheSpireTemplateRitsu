...
- 运行 `git submodule status -- local/MinionLib bundled_skills`。如果目录缺失，先执行 `git submodule update --init --recursive`。
...
- 对照 `CharMod.csproj` 里的 NuGet 版本，确认 `STS2.RitsuLib` 和 `FuYnAloft.Sts2.MinionLib` 的版本；RitsuLib 文档在 NuGet 包缓存的 `lib/net9.0/STS2-RitsuLib.xml`，MinionLib 源码在 `local/MinionLib/`。
...
- 实现卡牌或机制时，先查 `local/sts2-decompiled/`、RitsuLib NuGet XML 文档、`local/MinionLib/`，再参考 `CharModCode/Components/Enhances/` 和 `CharModCode/Cards/SampleEnhanceStrike.cs`。
