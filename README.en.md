...
After a developer gives the AI card text, the STS2 path, and expected test coverage, the AI can inspect `local/sts2-decompiled/`, RitsuLib package docs, `local/MinionLib/`, and nearby examples before changing code, localization, and tests. Tests run through MSBuild targets that launch STS2 in the headless TestTheSpire flow, so review output can point to the exact card and combat path that failed.
...
Initialize submodules in the new project. `local/MinionLib` and `bundled_skills` come from `.gitmodules`; RitsuLib comes from the `STS2.RitsuLib` NuGet package. The `dotnet new` output already contains `.agents/skills`; after submodule initialization, the upstream skill source is visible as well:
...
```text
Please follow AGENTS.md and check whether this repository is ready for AI development. The STS2 path is `/path/to/SlayTheSpire2`. Confirm submodules, local/sts2-decompiled, RitsuLib, MinionLib, bundled skills, and the TestTheSpire smoke test before implementing anything.
```
...
Please inspect nearby Attack examples, CharModCard, local/sts2-decompiled, RitsuLib package docs, and local/MinionLib before implementing, then run the focused test.
```
